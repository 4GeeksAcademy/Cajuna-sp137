        ...
        async function loadRequests() {
            ...
            try {
                const params = store.selectedCompany
                    ...
                    : ""
                const res = await fetch(import.meta.env.VITE_BACKEND_URL + "/api/vacation-requests" + params)
    ...
    return (
        <div className="container mt-4">
            <div className="d-flex justify-content-between align-items-center mb-3">
                ...
                <Link to="/vacation-requests/new" className="btn btn-success">+ New Request</Link>
            </div>
                ...
                <tbody>
                    {[...requests].sort((a, b) => a.id - b.id).map(req => (
                        <tr key={req.id}>
                            ...
                            <td>
                                <Link to={`/vacation-requests/${req.id}`} className="btn btn-sm btn-outline-primary me-1">Ver</Link>
                                <Link to={`/vacation-requests/${req.id}/edit`} className="btn btn-sm btn-outline-warning me-1">Editar</Link>
                                <Link to={`/vacation-requests/${req.id}/delete`} className="btn btn-sm btn-outline-danger">Eliminar</Link>
                            </td>
                        </tr>
                    ))}
                </tbody>
                ...
                {[...requests].sort((a, b) => a.id - b.id).map(req => (
                    <div key={req.id} className="card mb-2">
                        <div className="card-body py-2">
                            ...
                            <small className="text-muted">{new Date(req.start_date).toLocaleDateString()} → {new Date(req.end_date).toLocaleDateString()}</small>
                            <div className="mt-2">
                                <Link to={`/vacation-requests/${req.id}`} className="btn btn-sm btn-outline-primary me-1">Ver</Link>
                                <Link to={`/vacation-requests/${req.id}/edit`} className="btn btn-sm btn-outline-warning me-1">Editar</Link>
                                <Link to={`/vacation-requests/${req.id}/delete`} className="btn btn-sm btn-outline-danger">Eliminar</Link>
