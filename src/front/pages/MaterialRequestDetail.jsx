...
export const MaterialRequestDetail = () => {
    const { id } = useParams()
    ...
    return (
        <div className="container mt-4">
            ...
            <div className="card mb-3">
                <div className="card-body">
                    <dl className="row">
                        ...
                        <dd className="col-sm-9">{req.notes || "—"}</dd>

                        <dt className="col-sm-3">Created At</dt>
                        <dd className="col-sm-9">{new Date(req.created_at).toLocaleString()}</dd>

                        <dt className="col-sm-3">Updated At</dt>
                        <dd className="col-sm-9">{new Date(req.updated_at).toLocaleString()}</dd>
                    </dl>
                </div>
            </div>
            ...
            <div className="mt-3">
                <Link to="/material-requests" className="btn btn-secondary">Back to list</Link>
                <Link to={`/material-requests/${req.id}/edit`} className="btn btn-warning ms-2">Edit</Link>
                <Link to={`/material-requests/${req.id}/delete`} className="btn btn-danger ms-2">Delete</Link>
            </div>
        </div>
    )
}
