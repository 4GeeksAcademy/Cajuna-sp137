...
export const MedicalLeaveRequestList = () => {
        ...
        async function loadRequests() {
            ...
            try {
                const params = store.selectedCompany
                    ...
                    : ""
                const res = await fetch(import.meta.env.VITE_BACKEND_URL + "/api/medical-leave-requests" + params)
                const data = await res.json()
                setRequests(data)
            } catch (err) {
                console.error("Error fetching medical leave requests:", err)
            } finally {
                setLoading(false)
            }
        }
    ...
    return (
        <div className="container mt-4">
            <div className="d-flex justify-content-between align-items-center mb-3">
                <h1>Medical Leave Requests</h1>
                <Link to="/medical-leave-requests/new" className="btn btn-success">+ New Request</Link>
            </div>
                ...
                <tbody>
                    {[...requests].sort((a, b) => a.id - b.id).map(req => (
                        <tr key={req.id}>
                            ...
                            <td>{req.document || "—"}</td>
                            <td>
                                <Link to={`/medical-leave-requests/${req.id}`} className="btn btn-sm btn-outline-primary me-1">Ver</Link>
                                <Link to={`/medical-leave-requests/${req.id}/edit`} className="btn btn-sm btn-outline-warning me-1">Editar</Link>
                                <Link to={`/medical-leave-requests/${req.id}/delete`} className="btn btn-sm btn-outline-danger">Eliminar</Link>
                            </td>
                        </tr>
                    ))}
                </tbody>
